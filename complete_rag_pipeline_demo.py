#!/usr/bin/env python3
"""
Complete RAG Pipeline Demo - CasaDeAnza PolicyCopilot
===================================================
...
def load_and_chunk_documents():
    ...
    # Load policy documents from CasaDeAnza files
    docs, paths = read_casadeanza_docs()
    if not docs:
        print("⚠️ No CasaDeAnza documents found. Using embedded sample fallback.")
        docs = [
        ...
        paths = ["embedded_sample"]

    print(f"📄 Loaded {len(docs)} policy documents from files")
    ...
    # Chunk all documents
    all_chunks = []
    for doc_content, doc_path in zip(docs, paths):
    ...
    print(f"✂️ Created {len(all_chunks)} chunks from {len(docs)} documents")
...
def setup_vector_database(chunks: List[Dict]):
    ...
    try:
        collection = client.create_collection(
            name="casadeanza_policies",  # What is the collection name?
            metadata={"hnsw:space": "cosine"}  # What similarity metric is used?
        )
    except Exception:
        # Collection already exists, get it
        collection = client.get_collection("casadeanza_policies")
...
if __name__ == "__main__":
    print("🎯 CasaDeAnza PolicyCopilot - Complete RAG Pipeline Demo")
    print("=" * 60)
    ...
    test_queries = [
        ...
        "Patios and Balconies rules",
        "Every unit must maintain a smoke detector"
    ]
